...
<table align="center" width="100%">
  <tr>
    <td align="center" width="50%">
      ...
      <em><b>Figure 3(a): Authorized Monitoring Terminal View.</b> Live physical STM32 telemetry (COM10 @ 115,200 baud, 350 Hz): real-time descrambled Lead-II ECG, dynamic QRS tracking, live HRV cards, and calibrated acute stress inference ($\tau = 0.35$).</em>
    </td>
    <td align="center" width="50%">
...
Fixed global thresholds (e.g., classifying stress whenever $\text{Heart Rate} > 80$ BPM) fail across diverse populations because resting autonomic tone varies substantially:
* **Subject A (Athletic):** Resting HR = 52 BPM, Stress HR = 72 BPM ($\Delta\text{HR} = +38.5\%$)
* **Subject B (Sedentary):** Resting HR = 78 BPM, Stress HR = 96 BPM ($\Delta\text{HR} = +23.1\%$)
...
* **Cohort:** 15 healthy adult subjects (S2–S17, excluding non-existent S12; 12 males, 3 females; age: $27.5 \pm 2.4$ years).
...
* **$\text{SDNN} = \sqrt{\frac{1}{N-1} \sum_{i=1}^N (RR_i - \overline{RR})^2}$** (Total autonomic variability)
* **$\text{RMSSD} = \sqrt{\frac{1}{N-1} \sum_{i=1}^{N-1} (RR_{i+1} - RR_i)^2}$** (Parasympathetic vagal tone)
* **$\text{pNN50} = \frac{\text{Count}(|RR_{i+1} - RR_i| > 50\text{ ms})}{N-1} \times 100\%$** (High-frequency vagal modulation)
...
In each fold $k \in \{1, \dots, 15\}$, the classifier is trained strictly on 14 subjects. For the test subject $k$, resting baseline windows are utilized **solely for unsupervised relative calibration** ($X_k^* = (X_k - B_k) / |B_k|$); zero test stress labels are ever exposed during training.
...
| **Confusion Matrix** | **TN: 273, FP: 12<br>FN: 22, TP: 138** | Calibrated decision cutoff $\tau = 0.35$ |
...
<table align="center" width="100%">
  <tr>
    ...
    <td align="center" width="50%">
      ...
      <em><b>Figure 6(b): Cross-Validated ROC Curve.</b> ROC trajectory (AUC = 0.9494) with operating threshold $\tau = 0.35$ highlighted.</em>
    </td>
  </tr>
</table>
...
| **This Study - Personalized Model (Ours)** | **Chest ECG** | **Relative Baseline ($X^*$)** | **92.36%** | **89.03%** | **15-Fold LOSO ($\tau = 0.35$)** |
...
To interpret the learned decision boundary, we conducted a **30-repeat permutation importance analysis per fold** ($15 \times 30 = 450$ trials per feature) paired with standardized logistic regression odds ratios ($e^{w_i}$):
...
* **Cardiac Interval Compression ($\Delta\text{MeanRR}$):** Produced the largest discriminative degradation ($\Delta\text{AUC} = 0.1766$, $\Delta\text{F1} = 0.2321$, $\text{OR} = 0.0645$), confirming interval shortening as the dominant statistical marker of acute stress.
* **Heart Rate Acceleration ($\Delta\text{MeanHR}$):** Strongly elevates stress odds ($\text{OR} = 5.6453$, $\Delta\text{AUC} = 0.0203$, $\Delta\text{F1} = 0.1021$ in LR and $0.1401$ in Random Forest), consistent with sympathetic chronotropic activation during TSST cognitive challenge.
* **Interval Dispersion ($\Delta\text{pNN50}$):** Substantial predictive weight ($\text{OR} = 4.0784$, $\Delta\text{AUC} = 0.0596$, $\Delta\text{F1} = 0.0606$), reflecting rapid autonomic vagal modulation shifts during challenge.
* **Autonomic Total Variability ($\Delta\text{SDNN}$):** Acts as a protective calm indicator ($\text{OR} = 0.4625 < 1.0$, $\Delta\text{AUC} = 0.0078$, $\Delta\text{F1} = 0.0042$), physiologically compatible with preserved autonomic variability during homeostatic resting calm.
